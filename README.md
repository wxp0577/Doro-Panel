...
- **极简模式** - 纯净搜索页，独立壁纸和特效
- **默认启动模式设置** - 可选择打开网页时默认普通模式或极简模式，按用户隔离
...
- **左下角显示当前账号** - 登录后显示用户名，admin金色，管理员粉色
...
- **常见指数自动识别** - 输入000001自动识别为上证指数，不是平安银行
- **实时天气** - 自动IP定位或手动设置城市
- **热点资讯** - 每日60秒新闻，自动刷新
- **多引擎搜索** - 百度/谷歌/必应一键切换
...
### 🏠 首页常用站点
- **从其他分类选取** - 首页不再手动添加，而是从其他分类选取网站引用
- **内网/外网切换** - 支持分别设置内网和外网首页
- **16:10大UI选择器** - 支持搜索，图标小一点方便选择
- **删除不影响原网站** - 首页删除网站不影响其他分类的原网站
- **保存开关** - 避免误操作添加
...
# 指定版本
docker pull wxp0577/doro-panel:v1.1
```
...
- 点击左下角「👤 登录」按钮登录
- 默认账号：`admin` / `admin`
- 登录后左下角显示当前用户名，点击可退出登录
- admin账户不允许修改用户名
- admin账户可修改其他用户密码、升级/降级管理员

### 2. 添加网站
- 登录后，每个分类页面右上角点击「✏️ 编辑布局」进入编辑模式
- 点击「➕ 添加」添加新网站
...
- 常见指数代码会自动识别（上证指数、上证50、沪深300、中证500、科创50、深证成指、创业板指等）
- 可设置显示/隐藏，点击股票查看详情
...
**Q: 输入000001显示的是平安银行，不是上证指数？**
A: 已在v1.1修复，常见指数代码会自动识别。如果还是有问题，请升级到最新版本。
...
**Q: 容器无限重启？**
A: 查看日志 `docker logs doro-panel`，常见原因是数据目录权限问题，确保挂载的目录可写。v1.0已修复gunicorn多worker竞态条件导致的启动失败。

**Q: 如何查看系统日志？**
A: 管理员登录后，在「系统设置」→「日志」中查看，包含登录信息和操作记录。

**Q: 如何修改其他用户的密码？**
A: 仅admin账户（user_id=1）可以。在「系统设置」→「用户管理」中，对应用户点击「改密码」。

**Q: 默认启动模式怎么设置？**
A: 在「系统设置」→「通用」中设置「默认启动模式」，可选普通模式或极简模式，每个用户独立设置。
...
**v1.1 主要更新：**
- 👑 admin超级管理员权限管控（修改其他用户密码、升级/降级管理员）
- 🎯 默认启动模式设置（按用户隔离）
- 🔗 左下角GitHub项目链接
- 👤 左下角显示当前登录账号
- 🔒 分类上锁功能
...
**GitHub项目地址：** https://github.com/wxp0577/Doro-Panel

有问题或建议欢迎提交 [Issues](https://github.com/wxp0577/Doro-Panel/issues)
